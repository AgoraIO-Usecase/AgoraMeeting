...
export class AgoraRTCClient {
  ...
  setScreenProfile(profile: '720p_2' | '720p_1') {
    this._client.setScreenProfile(profile)
  }
  ...
  async publish(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const onSuccess = ({stream}: any) => {
        this._client.off('stream-published', ({evt}: any) => {})
        resolve(stream)
      ...
      this._client.on('stream-published', onSuccess)
      this._client.publish(this._localStream, onFailure)
    })
  }
  ...
  async unpublish() {
    return new Promise((resolve, reject) => {
      ...
      const onSuccess = (evt: any) => {
        this._client.off('stream-unpublished', (evt: any) => {})
        resolve()
      ...
      this._client.on('stream-unpublished', onSuccess)
      this._client.unpublish(this._localStream, onFailure)
    })
  }
