...
import {isSafari} from './ua-detector';
...
export class AgoraWebClient {
  ...
  async publishLocalStream(data: AgoraStreamSpec) {
    ...
    let stream = await this.rtc.publish();
  ...
  async startScreenShare ({
    ...
    try {
      ...
      return this.shareClient._localStream
    } catch(err) {
      throw err
    }
  }
  ...
  async exit () {
    ...
    if (this.shareClient) {
      try {
        ...
        errors.push({'shareClient unpublish': err});
      }
      try {
        await this.shareClient.leave();
      } catch (err) {
        errors.push({'shareClient leave': err});
      }
      try {
        this.shareClient.destroy();
        this.shareClient.destroyClient();
