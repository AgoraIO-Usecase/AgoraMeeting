...
export default class ScaleController extends React.Component<ScaleControllerProps, {}> {
  ...
  public render(): React.ReactNode {
      return (
        <>
        <span className="zoom_rate">{Math.ceil(this.props.zoomScale * 100)} %</span>
        <i className='zoom_minues' id='zoom_minues' onClick={() => this.moveRuleIndex(-1)}></i>
        <i className='zoom_plus' id='zoom_plus' onClick={() => this.moveRuleIndex(+1)}></i>
        </>
      );
  }
}