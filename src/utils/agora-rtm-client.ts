...
export enum PeerCmdMediaState {
  disable = 0,
  enable = 1
}

export enum PeerOperateAction {
  accept = 1,
  reject = 2
}
...
export default class AgoraRTMClient {
  ...
  async getChannelAttributeBy(channelName: string) {
    ...
    for (let key of Object.keys(json)) {
      if (key === 'teacher') {
        const teacherObj = jsonParse(get(json, `${key}.value`));
